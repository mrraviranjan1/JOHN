...
from pyJOHN.dB.asst_fns import *
from pyJOHN.dB.sudos import is_fullsudo
from pyJOHN.functions.helper import inline_mention
...
**JOHN [v{JOHN_version}](https://github.com/mrraviranjan1/JOHN), powered by @johnsupportuserbot**
"""
)
...
async def JOHN(event):
    ...
    if (event.sender_id != OWNER_ID) and not is_fullsudo(event.sender_id):
        ...
        if not udB.get("STARTMSG"):
            ...
            await event.reply(
                f"Hey there [{get_display_name(u)}](tg://user?id={u.id}), this is JOHN Assistant of [{JOHN_bot.me.first_name}](tg://user?id={JOHN_bot.uid})!\n\n{ok}",
            ...
            me = f"[{JOHN_bot.me.first_name}](tg://user?id={JOHN_bot.uid})"
