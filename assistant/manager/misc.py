...
@JOHN_cmd(pattern="decide", type="manager")
async def dheh(e):
...
@JOHN_cmd(pattern="echo ?(.*)", type=["manager"])
async def oqha(e):
...
@JOHN_cmd(pattern="kickme$", type=["manager"], allow_all=True)
async def doit(e):
...
@JOHN_cmd(pattern="joke$", type=["manager"])
async def do_joke(e):
